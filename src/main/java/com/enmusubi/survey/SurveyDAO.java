...
public class SurveyDAO {
	...
	public static void getMemberNormal(HttpServletRequest request) {
		...
		try {
			...
			// JSON 객체 생성
			JSONObject membersJson = new JSONObject(m);
			request.setAttribute("membersJson", membersJson.toString());
	...
	public static void selectLink(HttpServletRequest request) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			pstmt.setString(1, (String)request.getSession().getAttribute("eno"));
			rs=pstmt.executeQuery();
			if(rs.next()) {
				request.setAttribute("alink", rs.getString(1));
			}
			else {
				request.setAttribute("alink", "https://www.amazon.com/-/ko/ref=nav_logo");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
